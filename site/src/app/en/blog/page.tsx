    ...
    <main className="min-h-screen">
      <nav className="fixed top-0 w-full z-50 border-b border-white/10 bg-[#0a0a0a]/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 text-sm text-gray-400">
            ...
            <Link href="/en/blog" className="text-white">Blog</Link>
            <Link href="/blog" className="hover:text-white transition">中文</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="pt-32 pb-20 px-6 max-w-3xl mx-auto">
        ...
        <div className="space-y-8">
          {posts.map((post) => (
            <Link key={post.slug} href={`/en/blog/${post.slug}`} className="block group">
